...
1. Two proto files in the same package (`osac.private.v1`):
...
3. The instance type file imports the metadata type:
   - Input: `import "osac/private/v1/metadata_type.proto";`
   - Expected output: `import "osac/public/v1/metadata_type.proto";`
...
- ✅ Rename the package declaration from `osac.private.v1` to `osac.public.v1`
- ✅ Remove the private `metadata` field (which used the `Metadata` type)
- ✅ **Keep the import of `metadata_type.proto`** because `Metadata` is mentioned in comments
- ✅ Remove the cleanapi import
- ✅ Keep the remaining public fields

This demonstrates the current behavior where the import is retained because type names in comments are matched by the usage detection regex.

## Contrast with renamed_same_package_import

| Test Case | Import Path | Import Used? | Expected |
|-----------|-------------|--------------|----------|
| `renamed_same_package_import` | `osac/private/v1/instance_type.proto` → `osac/public/v1/instance_type.proto` | Yes (in public fields) | **KEPT** |
| `renamed_import_removal` | `osac/private/v1/metadata_type.proto` → (would be) `osac/public/v1/metadata_type.proto` | No (only in private field) | **REMOVED** |
